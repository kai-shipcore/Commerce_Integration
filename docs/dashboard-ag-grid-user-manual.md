...
날짜 선택에서 기준일을 바꾸면 **판매와 재고를 모두** 그 날짜 기준으로 다시 계산해서 볼 수 있습니다. S.O.D., 재고 수명, 컨테이너 체인의 시작점도 함께 그 날짜로 옮겨집니다.
...
### 과거 기준으로 바뀌는 것 / 바뀌지 않는 것

| | 과거 기준 | 비고 |
|---|---|---|
| 판매 전 구간 (90/60/30/15/7일, 평균) | ✅ | 판매 스냅샷에서 재집계 |
| Total Stock, Back | ✅ | 재고 이력에서 그 날짜 상태로 복원 |
| Fullerton / Canary / TTM / West / East 재고 | ❌ `—`로 표시 | 재고 이력이 SKU 합계만 기록해 창고별로 나눌 수 없습니다 |
| Transit Stock | ❌ 현재값 | 운송 기록에 상태 변경 이력이 없습니다 |
| Remaining / Mistake | ❌ 현재값 | 이력이 없습니다 |
| 컨테이너 목록·ETA·수량 | ❌ 현재값 | 컨테이너 수량은 이력이 남지 않습니다 |

즉 과거 날짜는 **"그 날의 판매·재고 × 현재의 운송·컨테이너 계획"** 입니다. 날짜 옆 안내 문구에 무엇이 현재값인지 표시되며, 마우스를 올리면 전체 설명이 나옵니다.

재고 이력에 그 날짜 기록이 없는 SKU는 현재 값을 그대로 씁니다. 이런 행은 창고별 재고가 `—`가 아니라 숫자로 남아 있어 구분됩니다.

### 선택할 수 있는 날짜 범위

너무 이른 날짜는 선택되지 않습니다. 판매 스냅샷과 재고 이력 중 **늦게 시작하는 쪽**이 하한이 되며, 판매 쪽은 이전 기간 비교를 위해 기준일보다 96일 더 뒤를 읽기 때문에 스냅샷 시작일보다 96일 늦은 날짜부터 온전한 값이 나옵니다. 하한 이전을 허용하면 90일 판매 윈도우가 잘린 채 조용히 낮은 숫자가 나오고, 그만큼 재고 수명이 길어 보입니다.
