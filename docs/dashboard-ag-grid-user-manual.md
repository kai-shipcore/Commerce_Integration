...
| 컬럼 | 의미 |
|---|---|
| West Stock | West 창고 재고 |
| East Stock | East 창고 재고 |
| Total Stock | 전체 재고 |
| Transit | 운송 중 재고. 클릭해서 수정 가능 |
| Mode | 재고 계산 모드. `OH`는 Onhand, `AV`는 Available |
...
| Mode | `OH` 또는 `AV` 클릭 | SKU별 stock mode |
