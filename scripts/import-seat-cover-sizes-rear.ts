// import-seat-cover-sizes-rear.ts
// Usage: npx tsx --env-file=.env scripts/import-seat-cover-sizes-rear.ts <path/to/file.xlsx> [--dry-run]
//
// Google Sheets > File > Download > XLSX 로 내보낸 파일을 읽어 shipcore.fc_seat_cover_parts_rear 에 upsert.
...
if (!filePath) {
  console.error("Usage: npx tsx --env-file=.env scripts/import-seat-cover-sizes-rear.ts <file.xlsx> [--dry-run]");
  process.exit(1);
}
...
async function main() {
  ...
  if (dryRun) {
    ...
    for (const row of dataRows.slice(0, 5)) {
      ...
      console.log(`  size=${size}  ymm=${cell(row, COL.ymm)}  package=${cell(row, COL.package)}  added_date=${cell(row, COL.addedDate)}`);
    }
    return;
  }
  ...
  for (const row of dataRows) {
    ...
    await pool.query(
      `INSERT INTO shipcore.fc_seat_cover_parts_rear (
        size, inventory, fitting_photo, confirmed, blueprint, manual, ymm,
        fitting_dp_detail, added_date, package,
