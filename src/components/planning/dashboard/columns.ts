...
function avgOrBlank(value: number | null | undefined): string {
...
export const ALL_COLS: ColDef[] = [
  ...
  { id: "status",    grp: "fix", label: "Sales\nStatus",    w: 72,  align: "ctr",  tint: "",        gh: "gh-fix",    val: (r) => ({ html: `<span class="sc ${r.sales_status === "Custom" ? "sc-cust" : r.sales_status === "Hold" ? "sc-hold" : r.sales_status === "Part" ? "sc-part" : r.sales_status === "Discontinued" ? "sc-disc" : r.sales_status === "TBD" ? "sc-tbd" : r.sales_status === "SWC" ? "sc-swc" : "sc-orig"}">${r.sales_status || ""}</span>` }), sortVal: (r) => r.sales_status ?? "" },
  ...
  { id: "fullerton", grp: "stock", label: "Fullerton\nStock", w: 64,  align: "num",  tint: "t-stock", gh: "gh-stock",  val: (r) => r.stock_mode === 'available' ? (r.fullerton_available_stock || 0) : (r.fullerton_stock || 0) },
  { id: "canary",    grp: "stock", label: "Canary\nStock",    w: 58,  align: "num",  tint: "t-stock", gh: "gh-stock",  val: (r) => r.stock_mode === 'available' ? (r.canary_available_stock    || 0) : (r.canary_stock    || 0) },
  { id: "ttm",       grp: "stock", label: "TTM\nStock",       w: 52,  align: "num",  tint: "t-stock", gh: "gh-stock",  val: (r) => r.stock_mode === 'available' ? (r.ttm_available_stock       || 0) : (r.ttm_stock       || 0) },
  { id: "ttm_jeff",  grp: "stock", label: "TTM Jeff\nStock",  w: 68,  align: "num",  tint: "t-stock", gh: "gh-stock",  val: (r) => r.stock_mode === 'available' ? (r.ttm_jeff_available_stock  || 0) : (r.ttm_jeff_stock  || 0) },
  ...
  { id: "total",     grp: "stock", label: "Total\nStock",     w: 50,  align: "num",  tint: "t-stock", gh: "gh-stock",  val: (r) => r.stock_mode === 'available' ? ((r.west_available_stock || 0) + (r.east_available_stock || 0) + (r.transit_stock || 0)) : (r.total_stock || 0), bold: true },
  ...
  { id: "wavg_c", grp: "wavg", label: "W Avg\n현재", w: 56, align: "num", tint: "t-avg", gh: "gh-avg", bold: true, val: (r) => {
    if (r.avg_daily_curr === null || r.avg_daily_curr === undefined) return "";
    const n = Math.round(r.avg_daily_curr * 100) / 100;
    const s = n.toFixed(2);
  ...
  // East Avg Daily
  { id: "eavg_p", grp: "eavg", label: "E Avg\n이전", w: 56, align: "num", tint: "t-avg", gh: "gh-avg", val: (r) => avgOrBlank(r.east_avg_prev), sortVal: (r) => r.east_avg_prev ?? -1 },
...
export const CON_SUBCOLS: ConSubColDef[] = [
  ...
  { id: "oo",        label: "Open\nOrders", w: 44, align: "num", tint: "t-cn",      val: (cd) => Math.round(cd.open_orders || 0) },
  { id: "avail", label: "Avail\nQty",   w: 44, align: "num", tint: "t-cn",      val: (cd) => cd.avail_qty !== null && cd.avail_qty !== undefined ? Math.round(cd.avail_qty) : "" },
  { id: "est",   label: "Est.\nSales",  w: 44, align: "num", tint: "t-cn",      val: (cd) => Math.round(cd.est_sales ?? 0) || 0 },
  { id: "cbo",   label: "Back\nOrder",  w: 44, align: "num", tint: "t-cn",      val: (cd) => {
    const v = Math.round(cd.backorder || 0);
