...
function detectCurrentStep(lines: string[]): number {
  ...
  for (const line of lines) {
    for (let i = 0; i < STEPS.length; i++) {
      if (line.includes(STEPS[i].marker) && i > current) current = i;
    }
  }
  return current;
}
...
export function RunForecast({ initialLastRun }: { initialLastRun: LastRun | null }) {
  ...
  const formattedLastRun = lastRun?.run_date
    ? new Date(lastRun.run_date).toLocaleString("en-US", {
    ...
    <Card>
      <CardHeader className="pb-3 pt-4">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-medium">Run Forecast</CardTitle>
          <span className="text-xs text-muted-foreground">
            Last run:{" "}
            {formattedLastRun
              ? `${formattedLastRun}${lastRun?.horizon_weeks ? ` · ${lastRun.horizon_weeks}W horizon` : ""}`
              : "—"}
          </span>
        </div>
      </CardHeader>
              ...
              <>
                ...
                {job.status === "done" && (
                  <p className="rounded-md bg-green-50 px-3 py-2 text-xs font-medium text-green-700 dark:bg-green-900/20 dark:text-green-400">
                    Forecast completed successfully. Segmentation data will refresh on next page load.
                  </p>
                )}
                {job.status === "failed" && (
                  <p className="rounded-md bg-red-50 px-3 py-2 text-xs font-medium text-red-700 dark:bg-red-900/20 dark:text-red-400">
                    Forecast failed. Check the output above for details.
                  </p>
                )}
                {job.status === "cancelled" && (
                  <p className="rounded-md bg-yellow-50 px-3 py-2 text-xs font-medium text-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-400">
                    Forecast cancelled. No data was written to the database.
                  </p>
                )}
              </>
            )}
