...
export const DemandPlanningRepository = {
  ...
  async getStatsRows(filters: DashboardFilters): Promise<Record<string, unknown>[]> {
    ...
    const result = await primary().query(`
      ...
      COALESCE(s.fba_90d_sales,  0)::float8                AS fba_90d_sales,
      COALESCE(s.fba_60d_sales,  0)::float8                AS fba_60d_sales,
      COALESCE(s.fba_30d_sales,  0)::float8                AS fba_30d_sales,
      COALESCE(s.fba_15d_sales,  0)::float8                AS fba_15d_sales,
      COALESCE(s.fba_7d_sales,   0)::float8                AS fba_7d_sales,
      COALESCE(s.fba_30d_pre,    0)::float8                AS fba_30d_pre,
      ...
      p.total_avg_prev_override::float8 AS total_avg_prev_override,
      p.total_avg_real_override::float8 AS total_avg_real_override,
