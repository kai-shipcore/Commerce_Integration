...
import { getCustomSalesForSkus, getCustomSalesTotals } from "@/lib/db/supabase-lookup";
...
async function buildEnrichResponse(skus: string[], search: string) {
  const [customMap, customTotalsRaw] = await Promise.all([
    skus.length ? getCustomSalesForSkus(skus) : Promise.resolve(new Map()),
    getCustomSalesTotals(search || undefined),
  ]);
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("[velocity/custom-enrich] GET error:", getErrorMessage(error));
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("[velocity/custom-enrich] POST error:", getErrorMessage(error));
    return NextResponse.json({ success: false, error: getErrorMessage(error) }, { status: 500 });
  }
}
